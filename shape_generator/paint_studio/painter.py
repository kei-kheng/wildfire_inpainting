...
def drawStimuli(FLAGS):
    ...
    all_idces = list(
        itertools.product(
            range(FLAGS.num_transformations), repeat=len(FLAGS.to_transform)
        )
    )
    ...
    for ii, stimulus_shape in enumerate(FLAGS.shapes):
        for jj, idx_tuple in enumerate(all_idces):
            ctx.save()
            ctx, img_mat, img = makeStimulus(
                img, FLAGS, idx_tuple, valueDict, img_centre, stimulus_shape, ctx, jj
            )
...
def makeStimulus(img, FLAGS, idces, valueDict, centre, stim_shape, ctx, imgID):
    ...
    print("-----new stimulus-------")
    for ii, dim in enumerate(
        FLAGS.to_transform
    ):  # iterate through dimensions to transform
    ...
    ctx = drawShape(ctx, FLAGS, [0, 0], stim_shape)
    tmp = np.frombuffer(
        img.get_data(), np.uint8
    )  # gets the raw pixel data in RGBA format.
    tmp.shape = [
        FLAGS.canvas_size[0],
        FLAGS.canvas_size[1],
        4,
    ]  # re-shaping to the dimension of the canvas
    tmp = tmp[:, :, 0:3]  # strips off the alpha channel, leaving just RGB.
    filename = f"img_{FLAGS.name[0]}_{imgID}.png"
    savename = os.path.join(FLAGS.outdir[0], filename)
    img.write_to_png(savename)
    return ctx, tmp, img
...
def drawShape(ctx, FLAGS, centre, stimShape):
    ...
    if stimShape == "rect":
        ...
        ctx = drawPolygon(
            ctx, centre, FLAGS.stim_poly_size, numVertices=int(stimShape[4:])
        )
    elif stimShape[0:4] == "star":
        ctx = drawStar(
            ctx,
            centre,
            FLAGS.stim_star_size[0],
            FLAGS.stim_star_size[1],
            numVertices=int(stimShape[4:]),
        )
    ...
    # elif stimShape == "ellipse":
    #     # If FLAGS.stim_ellipse_ratio is a single float, build a 2-element scale
    #     scale_tuple = (FLAGS.stim_ellipse_ratio, 1.0)
    #     ctx = drawEllipse(ctx, centre, FLAGS.stim_poly_size, scale_tuple)
