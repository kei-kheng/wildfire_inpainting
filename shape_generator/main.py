...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    default=["rect", "ellipse", "poly6", "star5"],
...
parser.add_argument(
    "--stim_poly_size", type=int, default=20, help='Polygon "Radius"  (default: 50)'
)
parser.add_argument(
    "--stim_star_size",
    type=float,
...
parser.add_argument(
    "--stim_rota", type=float, default=0, help="Stimulus Rotation (deg; default: 0)"
)
parser.add_argument(
    "--stim_trx",
    type=float,
...
parser.add_argument(
    ...
    nargs="+",
    default=[0.2, 2.0],
    help="Ellipse ratio range (min,max) to produce thin or fat ellipses",
)
...
def main(argv=None):
    ...
    data["idces"] = (
        all_idces  # for each image, store the feature values. (feature correspondence encoded in FLAGS.to_transform (same order))
    )
